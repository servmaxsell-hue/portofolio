<x-admin-layout>
    <x-slot name="title">Modifier: {{ $project->title }}</x-slot>

    <div class="max-w-3xl">
        <form action="{{ route('admin.projects.update', $project) }}" method="POST" class="bg-white rounded-xl shadow-sm p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" required
                              class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">{{ old('description', $project->description) }}</textarea>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="category" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500">
                            ...
                            <option value="automation" {{ $project->category == 'automation' ? 'selected' : '' }}>Automatisation</option>
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="featured" id="featured" value="1" {{ $project->featured ? 'checked' : '' }}
